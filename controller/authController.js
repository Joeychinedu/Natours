...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError(
        "You're are not logged in! Please log in to get access.",
        401
      )
    );
  }
  ...
  if (!currentUser) {
    return next(
      new AppError(
        'The user belonging to this user doess not exist anymore.',
        401
      )
    );
  }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.postedPassword, user.password))) {
